...
def scraper_boatos(max_pages: int = 50) -> list[FactCheck]:
    all_facts = []
    for page in range(1, max_pages + 1):
        ...
        if not html:
            break
        facts = parse_boatos_page(html, base_url=BASE_URL)
        if not facts:
            break
...
def parse_boatos_page(html: str, base_url: str) -> list[FactCheck]:
    ...
    for article in soup.select("article.post, article"):
        ...
        verdict_el = article.select_one(".classificacao, .verdict, [class*=verdict]")
        ...
        expl_el = article.select_one(".explicacao, .explanation, p")
        ...
        facts.append(FactCheck(
            titulo=title, veiculo="Boatos", url=url, data=data,
